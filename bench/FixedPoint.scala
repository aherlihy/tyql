...
object FixedPointQuery {
  ...
  final def fix[P](set: Boolean)(bases: Seq[P], acc: Seq[P])(fns: Seq[P] => Seq[P]): Seq[P] =
    ...
    val next = fns(bases)
    val nextAcc = (acc ++ bases)
    if (next.toSet.subsetOf(nextAcc.toSet))
      if (set) then nextAcc.distinct else nextAcc
    else
      val res = if (set) then nextAcc.distinct else nextAcc
  ...
  final def multiFix[T <: Tuple, S <: Seq[?]](set: Boolean, targetIdx: Int = 0)(bases: T, acc: T)(fns: (T, T) => T): T =
    ...
    val nextAcc = accA.zip(basesA).map((a: S, b: S) => if (set) then (a ++ b).distinct else a ++ b)


    val cmp = nextA.zip(nextAcc).map(
      (n, a) => n.toSet.subsetOf(a.toSet)
    ).forall(b => b)
    if (cmp)
      val res = Tuple.fromArray(nextAcc.toArray).asInstanceOf[T]
      res
    else
      val newAcc = Tuple.fromArray(nextAcc.toArray).asInstanceOf[T]
      val nextClean = nextA.zip(nextAcc).map((n, r) => n.filterNot(n => r.contains(n)))
...
//    println(s"ACC=${ScalaSQLTable.name(acc.asInstanceOf[Tuple2[ScalaSQLTable[?], ScalaSQLTable[?]]]._1)}")
  ...
  : Unit = {
    ...
    def printTable(t: Tables, name: String): Unit =
      println(s"${name}1(${ScalaSQLTable.name(t._1)})=${db.runRaw[(String)](s"SELECT * FROM ${ScalaSQLTable.name(t._1)}")}")
      println(s"${name}2(${ScalaSQLTable.name(t._2)})=${db.runRaw[(String, Int)](s"SELECT * FROM ${ScalaSQLTable.name(t._2)}")}")
      println(s"${name}3(${ScalaSQLTable.name(t._3)})=${db.runRaw[(String, Int, Double)](s"SELECT * FROM ${ScalaSQLTable.name(t._3)}")}")
    ...
    val copyInto: (Tables, Tables, Tables) => (Tables, Tables) = (base, acc, next) => {
      ...
      if (set)
        ...
        ddb.runUpdate(s"INSERT INTO ${tmp._3} (SELECT * FROM ${ScalaSQLTable.name(base._3)} UNION SELECT * FROM ${ScalaSQLTable.name(acc._3)})")
  ...
  : Unit = {
    ...
    val copyInto: (Tables, Tables, Tables) => (Tables, Tables) = (base, acc, next) => {
      ...
      if (set)
        ...
        ddb.runUpdate(s"INSERT INTO ${tmp._4} (SELECT * FROM ${ScalaSQLTable.name(base._4)} UNION SELECT * FROM ${ScalaSQLTable.name(acc._4)})")
