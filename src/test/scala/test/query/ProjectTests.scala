...
/** TODO: Concat doesn't work on these tests because the original types are defined as case classes. Should we have some automatic conversion from case class to
  * named tuple, or generate a named tuple from a case class + extra fields? Otherwise it's odd that you can't call concat on a normal row.
  */
