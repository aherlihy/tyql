...
class FlowFlatmapMapTest1 extends SQLStringQueryTest[AllCommerceDBs, (bName: String, pId: Int)] {
  ...
  def query() =
    testDB.tables.products.flatMap(p => testDB.tables.buyers.map(b => (bName = b.name, pId = p.id).toRow))
...
class FlowMapTest1 extends SQLStringQueryTest[AllCommerceDBs, (bName: String, bId: Int)] {
  def testDescription = "Flow: project tuple, 1 nest, map"
  def query() =
    testDB.tables.buyers.map(b => (bName = b.name, bId = b.id).toRow)
...
class FlowAggTest1b extends SQLStringAggregationTest[AllCommerceDBs, (bIdSum: Int, bIdMax: Int)] {
  ...
  def query() =
    testDB.tables.buyers.aggregate(b => (bIdSum = sum(b.id), bIdMax = max(b.id)).toRow)
...
class FlowFlatmapAggTest1b extends SQLStringAggregationTest[AllCommerceDBs, (bIdSum: Int, pIdMax: Int)] {
  ...
  def query() =
    testDB.tables.products.aggregate(p => testDB.tables.buyers.aggregate(b => (bIdSum = sum(b.id), pIdMax = max(p.id)).toRow))
...
class FlowFlatMapTest2 extends SQLStringQueryTest[AllCommerceDBs, (name: String, shippingDate: LocalDate)] {
  ...
  def query() =
    testDB.tables.buyers.flatMap(b => testDB.tables.shipInfos.map(si => (name = b.name, shippingDate = si.shippingDate).toRow))
  ...
  def query() =
    testDB.tables.shipInfos.flatMap(si1 =>
      testDB.tables.buyers.flatMap(b =>
        testDB.tables.shipInfos.map(si2 => (name = b.name, shippingDateA = si1.shippingDate, shippingDateB = si2.shippingDate).toRow)
      )
    )
...
class FlowMapFilterWithTest extends SQLStringQueryTest[AllCommerceDBs, (bName: String, bId: Int)] {
  ...
  def query() =
    testDB.tables.buyers.withFilter(_.id > 1).map(b => (bName = b.name, bId = b.id).toRow)
  def expectedQueryPattern = "SELECT buyers$A.name as bName, buyers$A.id as bId FROM buyers as buyers$A WHERE buyers$A.id > 1"
}
...
class FlowMapFilterTest extends SQLStringQueryTest[AllCommerceDBs, (bName: String, bId: Int)] {
  ...
  def query() =
    testDB.tables.buyers.filter(_.id > 1).map(b => (bName = b.name, bId = b.id).toRow)
  def expectedQueryPattern = "SELECT buyers$A.name as bName, buyers$A.id as bId FROM buyers as buyers$A WHERE buyers$A.id > 1"
}
...
class FlowMapSubsequentFilterTest extends SQLStringQueryTest[AllCommerceDBs, (bName: String, bId: Int)] {
  ...
  def query() =
    testDB.tables.buyers
      ...
      .filter(_.id > 100)
      .map(b => (bName = b.name, bId = b.id).toRow)
...
class FlowSubsequentMapTest extends SQLStringQueryTest[AllCommerceDBs, (bName3: String, bId3: Int)] {
  ...
  def query() =
    testDB.tables.buyers
      .map(b => (bName = b.name, bId = b.id).toRow)
      .map(b => (bName2 = b.bName, bId2 = b.bId).toRow)
      .map(b => (bName3 = b.bName2, bId3 = b.bId2).toRow)
  def expectedQueryPattern = """
    SELECT
...
class FlowMapFilterTest2 extends SQLStringQueryTest[AllCommerceDBs, (bName: String, bId: Int)] {
  ...
  def query() =
    testDB.tables.buyers.map(b => (bName = b.name, bId = b.id).toRow).filter(_.bId > 1) // for now generate subquery
...
class FlowMapAggregateTest extends SQLStringAggregationTest[AllCommerceDBs, Int] {
  ...
  def query() =
    testDB.tables.shipInfos.aggregate(si => sum(si.buyerId))
...
class FlowMapAggregateTest7 extends SQLStringAggregationTest[AllCommerceDBs, (sum: Int)] {
  ...
  def query() =
    testDB.tables.shipInfos.aggregate(si => (sum = sum(si.buyerId)).toRow)
