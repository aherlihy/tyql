...
class JoinSimple2Test extends SQLStringQueryTest[AllCommerceDBs, (id: Int, id2: Int)] {
  ...
  def expectedQueryPattern: String = """
  SELECT product$A.id as id, buyers$B.id as id2 FROM buyers as buyers$B, product as product$A WHERE product$A.name = buyers$B.name
  """
}
...
FlatMap(
  Filter(
    Table(buyers),
    Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant)))
  ),
  Fun(
    Ref(),
    Map(
      Filter(
        Table(product),
        Fun(Ref(),Eq(Select(Ref(),id),Select(Ref(),id)))
      ),
      Fun(
        Ref(),
        Project((Select(Ref(),name),Select(Ref(),name),Select(Ref(),price)))
      )
    )
  )
)
...
/*List(
  FromClause(
    TableLeaf(buyers/*Table(buyers)*/),
    WhereClause(
      List(
        BinExprOp(
          SelectExpr(name,QueryIRVar(TableLeaf(buyers,Table(buyers)),ref67/*Ref()*/)/*Select(Ref(),name)*/),
          Literal(string constant/*StringLit(string constant)*/),
          =,
          /*Eq(Select(Ref(),name),StringLit(string constant))*/
        )
      )
      /*Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant)))*/
    )
    /*Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))*/
  ),
  FromClause(
    TableLeaf(product/*,Table(product)*/),
    WhereClause(
      List(
        BinExprOp(
          SelectExpr(id,QueryIRVar(TableLeaf(product,Table(product)),ref69/*,Ref()*/),Select(Ref(),id)),
          SelectExpr(id,QueryIRVar(FromClause(TableLeaf(buyers,Table(buyers)),WhereClause(List(BinExprOp(SelectExpr(name,QueryIRVar(TableLeaf(buyers,Table(buyers)),ref67,Ref()),Select(Ref(),name)),Literal(string constant,StringLit(string constant)),=,Eq(Select(Ref(),name),StringLit(string constant)))),Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))),Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))),ref68,Ref()), Select(Ref(),id)),
          =,
          /*Eq(Select(Ref(),id),Select(Ref(),id))*/
        )
      ),
      /*Filter(Table(product),Fun(Ref(),Eq(Select(Ref(),id),Select(Ref(),id))))*/
    ),
    /*Filter(Table(product),Fun(Ref(),Eq(Select(Ref(),id),Select(Ref(),id))))*/
  )
),
projectIR=ProjectClause(List(AttrExpr(SelectExpr(name,QueryIRVar(FromClause(TableLeaf(buyers,Table(buyers)),WhereClause(List(BinExprOp(SelectExpr(name,QueryIRVar(TableLeaf(buyers,Table(buyers)),ref67,Ref()),Select(Ref(),name)),Literal(string constant,StringLit(string constant)),=,Eq(Select(Ref(),name),StringLit(string constant)))),Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))),Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))),ref68,Ref()),Select(Ref(),name)),Some(buyerName),Project((Select(Ref(),name),Select(Ref(),name),Select(Ref(),price)))), AttrExpr(SelectExpr(name,QueryIRVar(FromClause(TableLeaf(product,Table(product)),WhereClause(List(BinExprOp(SelectExpr(id,QueryIRVar(TableLeaf(product,Table(product)),ref69,Ref()),Select(Ref(),id)),SelectExpr(id,QueryIRVar(FromClause(TableLeaf(buyers,Table(buyers)),WhereClause(List(BinExprOp(SelectExpr(name,QueryIRVar(TableLeaf(buyers,Table(buyers)),ref67,Ref()),Select(Ref(),name)),Literal(string constant,StringLit(string constant)),=,Eq(Select(Ref(),name),StringLit(string constant)))),Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))),Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))),ref68,Ref()),Select(Ref(),id)),=,Eq(Select(Ref(),id),Select(Ref(),id)))),Filter(Table(product),Fun(Ref(),Eq(Select(Ref(),id),Select(Ref(),id))))),Filter(Table(product),Fun(Ref(),Eq(Select(Ref(),id),Select(Ref(),id))))),ref70,Ref()),Select(Ref(),name)),Some(productName),Project((Select(Ref(),name),Select(Ref(),name),Select(Ref(),price)))), AttrExpr(SelectExpr(price,QueryIRVar(FromClause(TableLeaf(product,Table(product)),WhereClause(List(BinExprOp(SelectExpr(id,QueryIRVar(TableLeaf(product,Table(product)),ref69,Ref()),Select(Ref(),id)),SelectExpr(id,QueryIRVar(FromClause(TableLeaf(buyers,Table(buyers)),WhereClause(List(BinExprOp(SelectExpr(name,QueryIRVar(TableLeaf(buyers,Table(buyers)),ref67,Ref()),Select(Ref(),name)),Literal(string constant,StringLit(string constant)),=,Eq(Select(Ref(),name),StringLit(string constant)))),Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))),Filter(Table(buyers),Fun(Ref(),Eq(Select(Ref(),name),StringLit(string constant))))),ref68,Ref()),Select(Ref(),id)),=,Eq(Select(Ref(),id),Select(Ref(),id)))),Filter(Table(product),Fun(Ref(),Eq(Select(Ref(),id),Select(Ref(),id))))),Filter(Table(product),Fun(Ref(),Eq(Select(Ref(),id),Select(Ref(),id))))),ref70,Ref()),Select(Ref(),price)),Some(price),Project((Select(Ref(),name),Select(Ref(),name),Select(Ref(),price))))),Project((Select(Ref(),name),Select(Ref(),name),Select(Ref(),price))))
*/
//
