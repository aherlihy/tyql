...
class SimpleExprTest extends SQLStringTest[AllCommerceDBs, Double] {
  def testDescription: String = "flatmap expr"

  def query() =
    testDB.tables.products
      .map(p => p.price)
  def sqlString: String = "SELECT purchase.price FROM purchase"
}

class SimpleExpr2Test extends SQLStringTest[AllCommerceDBs, (sum: Double)] {
  def testDescription: String = "flatmap with named tuple"

  def query() =
    testDB.tables.products
      .map(p => (sum = p.price).toRow)
  def sqlString: String = "SELECT purchase.price FROM purchase"
}
