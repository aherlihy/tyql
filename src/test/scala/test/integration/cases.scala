...
class CaseTests extends FunSuite {
  ...
  test("searched CASE") {
    ...
    checkExprDialect[String](
      cases(
        ...
        (lit(15) > lit(100)) -> lit("gt"),
        true -> lit("neither")),
      expectS("neither"), usesElse)(withDB.all)

    checkExprDialect[Int](
      cases(
        False -> lit(10),
        true -> lit(20)),
      expectI(20), usesElse)(withDB.all)
  ...
  test("simple CASE") {
    ...
    checkExprDialect[Boolean](
      ...
      expectB(true), usesElse)(withDB.all)
  }
