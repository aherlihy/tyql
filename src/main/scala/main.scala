...
@main def main =
  ...
  val q1 = cities.map: c => //   SELECT zipcode FROM cities,
    c.zipCode
  val q2 = cities.withFilter: city => // SELECT name FROM cities WHERE city.population > 10000
      city.population > 10_000
    .map: city =>
      city.name

  val q3 = // SELECT name from cities where city.population > 10000
    for
      city <- cities
      if city.population > 10_000
    yield city.name

  val q4 =  // SELECT city.* FROM cities AS city JOIN cities AS alt ON city.name=alt.name AND city.zipcode != alt.zipcode
    for
      city <- cities
      alt <- cities
  ...
  val q4_2 = // SELECT city.* FROM cities AS city JOIN cities AS alt ON city.name=alt.name AND city.zipcode != alt.zipcode
    for
      city <- cities
      alt <- cities2
      if city.name == alt.name && city.zipCode != alt.zipCode
  ...
  val q5 = // SELECT city.name AS name, addr.number AS num FROM cities AS city JOIN addresses AS addr ON addr.street = city.name
    for
      city <- cities
      addr <- addresses
  ...
  val q6 = // SELECT city.name AS name, city.zipcode AS zipcode FROM cities AS city
    cities.map: city =>
