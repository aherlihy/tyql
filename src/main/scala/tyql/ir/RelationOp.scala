...
trait RelationOp extends QueryIRNode:
  ...
  /** Avoid overloading TODO: decide if we want to mutate, or copy + discard IR nodes. Right now its a mix, which should be improved
    */
  ...
  /** Add some extra metadata needed to generate "nice" SQL strings. Also used to handle edge cases, e.g. expression-level relation-ops should get aliases, etc.
    * @param f
    *   \- the flag
    */
...
/** Select query, e.g. SELECT * FROM <Relations> WHERE <where?> Separate from SelectQuery because SELECT * queries can be unnested
  */
case class SelectAllQuery(from: Seq[RelationOp], where: Seq[QueryIRNode], overrideAlias: Option[String], ast: DatabaseAST[?]) extends RelationOp:
  ...
  override def toSQLString(): String =
    ...
    val whereStr =
      if where.nonEmpty then s" WHERE ${if where.size == 1 then where.head.toSQLString() else where.map(f => f.toSQLString()).mkString("(", " AND ", ")")}"
      else ""
    wrapString(
...
/** Select query, e.g. SELECT <Project> FROM <Relations> WHERE <where> TODO: Eventually specialize join nodes, for now use Select with multiple FROM fields to
  * indicate join. Right now the AST only supports nested flatMaps, will eventually expand syntax to support specifying join type.
  */
case class SelectQuery(project: QueryIRNode, from: Seq[RelationOp], where: Seq[QueryIRNode], overrideAlias: Option[String], ast: DatabaseAST[?])
    extends RelationOp:
  val name = overrideAlias.getOrElse({
    val latestVar = s"subquery${QueryIRTree.idCount}"
    QueryIRTree.idCount += 1
...
case class OrderedQuery(query: RelationOp, sortFn: Seq[(QueryIRNode, Ord)], ast: DatabaseAST[?]) extends RelationOp:
  ...
  override def toSQLString(): String =
    wrapString(s"${query.toSQLString()} ORDER BY ${sortFn
        .map(s =>
...
case class GroupByQuery(source: RelationOp, groupBy: QueryIRNode, having: Option[QueryIRNode], overrideAlias: Option[String], ast: DatabaseAST[?])
    extends RelationOp:
  val name = overrideAlias.getOrElse({
    val latestVar = s"subquery${QueryIRTree.idCount}"
    QueryIRTree.idCount += 1
  ...
  override def toSQLString(): String =
    ...
    val havingStr = if having.nonEmpty then s" HAVING ${having.get.toSQLString()}" else ""
