...
case class WhereClause(children: Seq[QueryIRNode], ast: Expr[?, ?, ?]) extends QueryIRNode:
  override val schema = ResultTag.AnyTag
 ...
 */
case class BinExprOp(lhs: QueryIRNode, rhs: QueryIRNode, op: (String, String) => String, schema: ResultTag[?], ast: Expr[?, ?, ?]) extends QueryIRNode:
 ...
 * TODO: cannot assume the operation is universal, need to specialize for DB backend
 */
case class UnaryExprOp(child: QueryIRNode, op: String => String, schema: ResultTag[?], ast: Expr[?, ?, ?]) extends QueryIRNode:
 ...
 * @param ast
 */
case class ProjectClause(children: Seq[QueryIRNode], schema: ResultTag[?], ast: Expr[?, ?, ?]) extends QueryIRNode:
 ...
 */
case class AttrExpr(child: QueryIRNode, projectedName: Option[String], schema: ResultTag[?], ast: Expr[?, ?, ?]) extends QueryIRNode:
 ...
 */
case class SelectExpr(attrName: String, from: QueryIRNode, schema: ResultTag[?], ast: Expr[?, ?, ?]) extends QueryIRLeaf:
...
case class QueryIRVar(toSub: RelationOp, name: String, ast: Expr.Ref[?, ?, ?]) extends QueryIRLeaf:
  override val schema = toSub.schema
 ...
 */
case class Literal(stringRep: String, schema: ResultTag[?], ast: Expr[?, ?, ?]) extends QueryIRLeaf:
  override def toSQLString(): String = stringRep

case class ListTypeExpr(elements: List[QueryIRNode], ast: Expr[?, ?, ?]) extends QueryIRNode:
  override val schema = ResultTag.ListTag(elements.head.schema)
