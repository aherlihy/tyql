...
/** Named or unnamed attribute select expression, e.g. `table.rowName as customName` TODO: generate anonymous names, or allow generated queries to be unnamed,
  * or only allow named tuple results? Note projected attributes with names is not the same as aliasing, and just exists for readability
  */
