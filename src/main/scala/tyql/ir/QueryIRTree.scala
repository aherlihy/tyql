...
//    println(s"genQuery: ast=$ast")
  ...
  private def generateExpr(ast: Expr[?], symbols: SymbolTable): QueryIRNode =
    ast match
      ...
      case g: Expr.Gte => BinExprOp(generateExpr(g.$x, symbols), generateExpr(g.$y, symbols), (l, r) => s"$l >= $r", g)
      ...
      case g: Expr.LteDouble => BinExprOp(generateExpr(g.$x, symbols), generateExpr(g.$y, symbols), (l, r) => s"$l <= $r", g)
      case g: Expr.GteDouble => BinExprOp(generateExpr(g.$x, symbols), generateExpr(g.$y, symbols), (l, r) => s"$l >= $r", g)
      case g: Expr.GtDate => BinExprOp(generateExpr(g.$x, symbols), generateExpr(g.$y, symbols), (l, r) => s"$l > $r", g)
      case g: Expr.LtDate => BinExprOp(generateExpr(g.$x, symbols), generateExpr(g.$y, symbols), (l, r) => s"$l < $r", g)
      case g: Expr.LteDate => BinExprOp(generateExpr(g.$x, symbols), generateExpr(g.$y, symbols), (l, r) => s"$l <= $r", g)
      case g: Expr.GteDate => BinExprOp(generateExpr(g.$x, symbols), generateExpr(g.$y, symbols), (l, r) => s"$l >= $r", g)
      ...
      case a: Expr.Minus[?] => BinExprOp(generateExpr(a.$x, symbols), generateExpr(a.$y, symbols), (l, r) => s"($l - $r)", a)
      ...
      case l: Expr.DateLit => Literal(s"DATE '${l.$value}'", l)
  ...
  private def generateAggregation(ast: AggregationExpr[?], symbols: SymbolTable): QueryIRNode =
    ast match
      ...
      case c: AggregationExpr.CountAll => Literal("COUNT(*)", c)
