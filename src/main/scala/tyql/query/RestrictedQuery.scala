...
/** A restricted reference to a query that disallows aggregation. Explicitly do not export aggregate, or any aggregation helpers, exists, etc.
  *
  * Methods can accept RestrictedQuery[A] or Query[A] NOTE: Query[?] indicates no aggregation, but could turn into aggregation, RestrictedQuery[?] means none
  * present and none addable.
  *
  * flatMap/union/unionAll/etc. that accept another RestrictedQuery contain a contextual parameter ev that serves as an affine recursion restriction, e.g. every
  * input relation can only be "depended" on at most once per query since the dependency set parameter `D` must be disjoint.
  */
