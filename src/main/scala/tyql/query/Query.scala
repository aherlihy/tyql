...
trait Query[A, Category <: ResultCategory](using ResultTag[A]) extends DatabaseAST[A]:
  ...
  /** Classic flatMap with an inner query that is a RestrictedQuery. This turns the result query into a RestrictedQuery. Exists to support doing
    * BaseCaseRelation.flatMap(...) within a fix
    * @param f
  ...
  /** Equivalent to aggregate(f: Ref => Aggregation). NOTE: make Ref of type NExpr so that relation.id is counted as NExpr, not ScalarExpr
    *
    * @param f
  ...
  /** Equivalent version of map(f: Ref => Aggregation). Requires f to call toRow on the final result before returning. Sometimes the implicit conversion kicks
    * in and converts a named-tuple-of-Agg into a Agg-of-named-tuple, but not always. As an alternative, can use the aggregate defined below that explicitly
    * calls toRow on the result of f.
    *
    * @param f
    *   a function that returns an Aggregation (guaranteed agg in subtree)
    * @tparam B
  ...
  /** A version of the above-defined aggregate that allows users to skip calling toRow on the result in f.
    ...
    */
  def aggregate[B <: AnyNamedTuple: AggregationExpr.IsTupleOfAgg] /*(using ev: AggregationExpr.IsTupleOfAgg[B] =:= true)*/ (using
      ResultTag[NamedTuple.Map[B, Expr.StripExpr]]
  )(f: Ref[A, NonScalarExpr] => B): Aggregation[NamedTuple.Map[B, Expr.StripExpr]] =
...
//  Right now separated due to issues with overloading, but in theory could be condensed into a single groupBy method
  /** groupBy where the grouping clause is NOT an aggregation. Can add a 'having' statement incrementally by calling .having on the result. The selectFn MUST
    * return an aggregation.
    *
    * @param groupingFn
    *   \- must be a named tuple, in order from left->right. Must return an non-scalar expression.
    * @param selectFn
    *   \- the project clause of the select statement that is grouped NOTE: filterFn - the HAVING clause is used to filter groups after the GROUP BY operation
    *   has been applied. filters applied before the groupBy occur in the WHERE clause.
    * @tparam R
...
//  (using ev: Tuple.Union[GetFields[A]] <:< Tuple.Union[GetFields[G]])
  ...
  /** groupBy where the grouping clause IS an aggregation. Can add a 'having' statement incrementally by calling .having on the result. The selectFn MUST return
    * an aggregation.
    *
    * @param groupingFn
  ...
  /** filter based on a groupBy result. The selectFn MUST NOT return an aggregation. The groupingFn MUST NOT return an aggregation. The filterFn MUST return an
    * aggregation.
    *
    * @param groupingFn
...
//    Query.GroupBy(this, groupFun, selectFun, None)
  ...
  /** Classic map with an inner expression to transform the row. Requires f to call toRow on the final result before returning. Sometimes the implicit
    * conversion kicks in and converts a named-tuple-of-Expr into a Expr-of-named-tuple, but not always. As an alternative, can use the map defined below that
    * explicitly calls toRow on the result of f.
    *
    * @param f
  ...
  /** A version of the above-defined map that allows users to skip calling toRow on the result in f.
    ...
    */
  def map[B <: AnyNamedTuple: Expr.IsTupleOfExpr](using ResultTag[NamedTuple.Map[B, Expr.StripExpr]])(
...
object Query:
  ...
  /** Fixed point computation.
    *
    * @param bases
    *   Tuple of the base case queries. NOTE: bases do not have to be sets since they will be directly fed into an union. TODO: in theory, the results could be
    *   assumed to be sets due to the outermost UNION(base, recur), but better to enforce .distinct (also bc we flatten the unions)
    * @param fns
    *   A function from a tuple of restricted query refs to a tuple of *SET* restricted queries.
    *
    * QT is the tuple of Query DT is the tuple of tuple of constant ints. Used to track dependencies of recursive definitions in order to enforce that every
    * recursive reference is used at least once. RQT is the tuple of RestrictedQuery that should be returned from fns.
    *
    * NOTE: Because groupBy is a Query not an Aggregation, the results of groupBy are allowed as base-cases of recursive queries. This is consistent with
    * stratified aggregation. However, if you wanted to prevent recursion of any kind within mutual recursive queries (e.g. postgres), would need to add a
    * constraint to prevent fix(...) from taking in anything of type GroupBy within QT. Not applicable to inline fix, since the restriction is only for mutual
    * recursion.
    */
  def fix[QT <: Tuple, DT <: Tuple, RQT <: Tuple](bases: QT)(fns: ToRestrictedQueryRef[QT] => RQT)(using
      @implicitNotFound("Number of base cases must match the number of recursive definitions returned by fns") ev0: Tuple.Size[QT] =:= Tuple.Size[RQT]
  )(using @implicitNotFound("Base cases must be of type Query: ${QT}") ev1: Union[QT] <:< Query[?, ?])(using
      @implicitNotFound("Cannot constrain DT") ev2: DT <:< InverseMapDeps[RQT]
  )(using @implicitNotFound("Recursive definitions must be linear: ${RQT}") ev3: RQT <:< ToRestrictedQuery[QT, DT])(using
      @implicitNotFound(
...
//                                    (using @implicitNotFound("Recursive definitions must be linear, e.g. recursive references cannot appear twice within the same recursive definition: ${RQT}") ev2: Tuple.Union[Tuple.Map[DT, CheckDuplicate]] =:= false)
  ...
  def unrestrictedFixImpl[QT <: Tuple, P <: Tuple, R <: Tuple](bases: QT)(fns: P => R): ToQuery[QT] =
    ...
    val recursiveDefsList: List[Query[?, ?]] = recurQueries.toList
      .map(_.asInstanceOf[Query[?, ?]])
      .lazyZip(baseCaseDefsList)
      .map:
  ...
  def fixImpl[QT <: Tuple, P <: Tuple, R <: Tuple](bases: QT)(fns: P => R): ToQuery[QT] =
    ...
    val baseCaseDefsList = baseRefsAndDefs.map(_._2.asInstanceOf[Query[?, ?]])
    val recursiveDefsList: List[Query[?, ?]] = recurQueries.toList
      .map(_.asInstanceOf[RestrictedQuery[?, ?, ?]].toQuery)
      .lazyZip(baseCaseDefsList)
      .map:
        case (query: Query[t, c], ddef) =>
          // Optimization: remove any extra .distinct calls that are getting fed into a union anyway
  ...
  case class MultiRecursive[R]($param: List[RestrictedQueryRef[?, ?, ?]], $subquery: List[Query[?, ?]], $resultQuery: Query[R, ?])(using ResultTag[R])
      extends Query[R, SetResult]
  ...
  case class GroupBy[
    ...
    def having(p: Ref[SourceType, ?] => Expr[Boolean, ?]): Query[ResultType, BagResult] =
      ...
      else throw new Exception("Error: can only support a single having statement after groupBy")
