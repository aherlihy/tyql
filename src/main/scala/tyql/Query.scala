...
//    Aggregation.AggFlatMap(this, Expr.Fun(ref, f(ref)))
  ...
  inline def map[B: ResultTag](f: Expr.Ref[A, true] => Query[B]): Nothing =
...
//
//  case class Contains[A]($this: Query[A], $other: Expr[A, ?]) extends Expr[Boolean, false]
//  case class IsEmpty[A]($this: Query[A]) extends Expr[Boolean, false]
//  case class NonEmpty[A]($this: Query[A]) extends Expr[Boolean, false]
...
//                           $selectFn: Fun[A, Expr[B, false], false],
