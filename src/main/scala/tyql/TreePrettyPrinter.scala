...
object TreePrettyPrinter {
  ...
  private def indent(level: Int): String =
    scala.collection.StringOps("  ") * level // without explicit StringOps, treated as Expr[String] * Int
  ...
  extension (expr: Expr[?, ?]) {
    def prettyPrint(depth: Int): String = expr match {
      ...
      case Xor(x, y) =>
        s"${indent(depth)}Xor(\n${x.prettyPrint(depth + 1)},\n${y.prettyPrint(depth + 1)}\n${indent(depth)})"
      case Modulo(x, y) =>
        s"${indent(depth)}Modulo(\n${x.prettyPrint(depth + 1)},\n${y.prettyPrint(depth + 1)}\n${indent(depth)})"
      case Cast(x, resultType) =>
      ...
      case StrLike(s, pattern) =>
        s"${indent(depth)}StrLike(\n${s.prettyPrint(depth + 1)},\n${pattern.prettyPrint(depth + 1)}\n${indent(depth)})"
      case StrConcat(x, xs) =>
        s"${indent(depth)}StrConcat(\n${x.prettyPrint(depth + 1)},\n${xs.map(_.prettyPrint(depth + 1)).mkString(",\n")}\n${indent(depth)})"
      case StrConcatUniform(x, xs) =>
        s"${indent(depth)}StrConcatUniform(\n${x.prettyPrint(depth + 1)},\n${xs.map(_.prettyPrint(depth + 1)).mkString(",\n")}\n${indent(depth)})"
      case StrConcatSeparator(sep, x, xs) =>
        s"${indent(depth)}StrConcatSeparator(\n${sep.prettyPrint(depth + 1)},\n${x.prettyPrint(depth + 1)},\n${xs.map(_.prettyPrint(depth + 1)).mkString(",\n")}\n${indent(depth)})"
      case StrReverse(x) => s"${indent(depth)}StrReverse(${x.prettyPrint(0)})"
      case StrRepeat(s, n) =>
        s"${indent(depth)}StrRepeat(\n${s.prettyPrint(depth + 1)},\n${n.prettyPrint(depth + 1)}\n${indent(depth)})"
      case StrLPad(s, len, pad) =>
        s"${indent(depth)}StrLPad(\n${s.prettyPrint(depth + 1)},\n${len.prettyPrint(depth + 1)},\n${pad.prettyPrint(depth + 1)}\n${indent(depth)})"
      case StrRPad(s, len, pad) =>
        s"${indent(depth)}StrRPad(\n${s.prettyPrint(depth + 1)},\n${len.prettyPrint(depth + 1)},\n${pad.prettyPrint(depth + 1)}\n${indent(depth)})"
      case StrPositionIn(substr, string) =>
        s"${indent(depth)}StrPositionIn(\n${substr.prettyPrint(depth + 1)},\n${string.prettyPrint(depth + 1)}\n${indent(depth)})"
      case NullSafeEq(x, y) =>
        s"${indent(depth)}NullSafeEq(\n${x.prettyPrint(depth + 1)},\n${y.prettyPrint(depth + 1)}\n${indent(depth)})"
      case NullSafeNe(x, y) =>
        s"${indent(depth)}NullSafeNe(\n${x.prettyPrint(depth + 1)},\n${y.prettyPrint(depth + 1)}\n${indent(depth)})"
      case Round(x) => s"${indent(depth)}Round(${x.prettyPrint(0)})"
      case RoundWithPrecision(x, precision) =>
        s"${indent(depth)}RoundWithPrecision(\n${x.prettyPrint(depth + 1)},\n${precision.prettyPrint(depth + 1)}\n${indent(depth)})"
      case Ceil(x)  => s"${indent(depth)}Ceil(${x.prettyPrint(0)})"
      case Floor(x) => s"${indent(depth)}Floor(${x.prettyPrint(0)})"
      case Power(x, y) =>
        s"${indent(depth)}Power(\n${x.prettyPrint(depth + 1)},\n${y.prettyPrint(depth + 1)}\n${indent(depth)})"
  ...
  extension (node: QueryIRNode) {
    def prettyPrintIR(depth: Int, printAST: Boolean): String = node match {
      ...
      case searchedCase: SearchedCaseOp =>
        ...
        val elsePrint = searchedCase.elseClause.map(e =>
          s"\n${indent(depth + 1)}ELSE:\n${e.prettyPrintIR(depth + 2, printAST)}"
        ).getOrElse("")
        val astPrint =
      ...
      case simpleCase: SimpleCaseOp =>
        ...
        val elsePrint = simpleCase.elseClause.map(e =>
          s"\n${indent(depth + 1)}ELSE:\n${e.prettyPrintIR(depth + 2, printAST)}"
        ).getOrElse("")
        val astPrint =
