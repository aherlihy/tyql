...
// POC of the type-level helpers used by `insertInto`.  The full
// implementation lives on the `backend-specialization` branch.
...
object TypeOperations:
  ...
  // XXX Scala compiler will INCORRECTLY compute this if you change it even a
  // little, this is the only working version after 3+ hours of trying.
  // XXX Do not split this into helper types, do not use nested type matching,
  // do not abstract it, only extend this by example.
  ...
  type IsAcceptableInsertion[TypesA <: Tuple, TypesB <: Tuple] =
    AcceptableInsertions[TypesA, TypesB] =:= true
