...
import Expr.{Fun, StripExpr}

/**
 * A scalar operation, e.g. an expression that returns a single result, instead of a collection.
 */
trait AggregationExpr[Result](using ResultTag[Result]) extends Expr[Result]
...
// For now restrict all elements to be agg instead of allowing a mix.
  type IsTupleOfAgg[A <: AnyNamedTuple] = Tuple.Union[NamedTuple.DropNames[A]] <:< Expr[?]
  ...
  extension [A <: AnyNamedTuple, B <: Tuple](using B <:< NamedTuple.DropNames[A])(x: A)
