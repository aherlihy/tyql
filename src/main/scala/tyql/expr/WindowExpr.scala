...
case class WindExpr[R]
  (
      ae: Either[AggregationExpr[R], ExprInWindowPosition[R]],
      partitionBy: Seq[Expr[?, NonScalarExpr]],
      orderBy: Seq[(Expr[?, NonScalarExpr], tyql.Ord)]
  )
...
case class Lag[R, S <: ExprShape](e: Expr[R, S], offset: Option[Int], default: Option[Expr[R, S]])(using ResultTag[R])
    extends ExprInWindowPosition[R]
case class Lead[R, S <: ExprShape](e: Expr[R, S], offset: Option[Int], default: Option[Expr[R, S]])(using ResultTag[R])
    extends ExprInWindowPosition[R]
